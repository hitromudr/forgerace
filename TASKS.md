...
### TASK-093: Логика наполнения истории в pipeline.py
- **Статус**: open
- **Приоритет**: P1
- **Этап**: 2
- **Зависимости**: TASK-091
- **Файлы (новые)**: —
- **Файлы (modify)**: forgerace/pipeline.py
- **Интеграция**: В функциях `execute_task_competitive` и `execute_task_single` добавить сохранение данных в `task.last_attempts` перед вызовом `send_to_rework`.
- **Описание**: При получении вердикта `NEEDS_REWORK` (или любого не-APPROVED), необходимо инкрементировать `task.rework_count`, собирать комментарии всех ревьюеров и текущий `git diff --stat`, и сохранять это в историю задачи.
- **Критерий готовности**: После каждого раунда доработки в `TASKS.md` обновляется счетчик и добавляется запись в историю попыток.
