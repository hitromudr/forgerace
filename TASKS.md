...
### TASK-106: Обновление состояния задачи в цикле реворка
- **Статус**: open
- **Приоритет**: P1
- **Этап**: 2
- **Зависимости**: TASK-104
- **Файлы (новые)**: —
- **Файлы (modify)**: forgerace/pipeline.py
- **Интеграция**: —
- **Описание**: Интегрировать в цикл реворка инкремент `task.rework_count` и сохранение текста каждого нового замечания в список `task.last_attempts`.
- **Запрещено**: Очищать историю `last_attempts` между итерациями в рамках одной задачи.
- **Проверка**: python3 -m pytest tests/test_pipeline.py
