...
### TASK-112: Поддержка специализированных фреймов в forgerace/agents.py
- **Статус**: open
- **Приоритет**: P2
- **Этап**: 1
- **Зависимости**: —
...
- **Файлы (modify)**: forgerace/agents.py
- **Интеграция**: —
- **Описание**: Реализовать поддержку системных фреймов `+architect` и `+theory` для агентов. Обеспечить механизм передачи контекста "планировщика" в метаданные агента, чтобы модель понимала свою роль как проектировщика графа зависимостей.
- **Запрещено**: Использовать goose-модели для ролей, требующих сложного JSON-вывода.
- **Проверка**: `ruff check forgerace/agents.py`
