...
- **Файлы (новые)**: —
- **Файлы (modify)**: forgerace/task_cmd.py
- **Интеграция**: Добавить вызов `list_tasks()` в `cli.py`
- **Описание**: Создать функцию `list_tasks()` в `forgerace/task_cmd.py`, которая выводит компактную таблицу задач: ID, статус, агент, название. Использовать `parse_tasks()` из `tasks.py` для парсинга `TASKS.md`.
- **Запрещено**: Использовать внешние зависимости
- **Проверка**: `make check`
- **Критерий готовности**: Команда `./fr task list` выводит компактную таблицу задач
- **Дискуссия**: task-add-cmd
- **Агент**: aider-gptoss
