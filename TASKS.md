...
### TASK-119: Интеграция сбора метрик в Pipeline
- **Статус**: open
- **Приоритет**: P1
- **Этап**: 2
...
- **Описание**: Интегрировать сбор метрик в процесс выполнения задач (`run_task`/`execute_step`). Замерять время выполнения через таймер, получать стоимость из `TokenUsage`, считать раунды ревью. После завершения задачи вычислять `lines_changed` через `git diff --shortstat`. Сохранять итоговый `BenchmarkRecord` через `BenchmarkStore`.
- **Запрещено**: не блокировать основной поток выполнения при ошибках сбора метрик (использовать try/except), не вызывать `git diff` если репозиторий не инициализирован.
- **Проверка**: `ruff check forgerace/pipeline.py`
- **Критерий готовности**: Каждый запуск задачи в пайплайне автоматически создает запись в `.agents/benchmark.json`.
