...
- **Интеграция**: —
- **Описание**: Добавить поля `rework_count` (int) и `last_attempts` (str) в dataclass `Task`. Обновить регулярные выражения в `parse_tasks` и логику записи в `update_task_status` для корректной поддержки этих полей в Markdown-файле `TASKS.md`.
- **Критерий готовности**: Новые поля успешно парсятся из `TASKS.md` и сохраняются обратно при вызове `update_task_status` без потери данных.
- **Дискуссия**: championship-smart-rework
- **Агент**: —
- **Ветка**: —

### TASK-099: Реализация логики формирования rework-промптов
- **Статус**: open
- **Приоритет**: P1
- **Этап**: 1
...
- **Интеграция**: —
- **Описание**: Реализовать функцию `build_rework_prompt(task)`, которая генерирует текстовый блок с историей предыдущих попыток (счетчик, имя агента, замечания). Интегрировать её в `build_prompt`, чтобы при `attempt > 1` этот блок вставлялся в системный промпт агента.
- **Критерий готовности**: Функция `build_prompt` возвращает промпт, содержащий историю ревью, если у задачи `rework_count > 0`.
- **Дискуссия**: championship-smart-rework
- **Агент**: —
- **Ветка**: —

### TASK-100: Интеграция цикла реворка в основной пайплайн
- **Статус**: open
- **Приоритет**: P1
- **Этап**: 1
...
- **Файлы (modify)**: forgerace/pipeline.py
- **Интеграция**: —
- **Описание**: В `pipeline.py` в логике обработки `rework` (после ревью) добавить: инкремент `task.rework_count`, формирование строки с комментариями ревьюера, именем агента и таймстампом для `task.last_attempts`. Вызывать `update_task_status` для сохранения состояния в файл перед повторным запуском.
- **Критерий готовности**: При каждой неудачной проверке счетчик в `TASKS.md` увеличивается, история обновляется, и следующий агент получает актуальные замечания.
- **Дискуссия**: championship-smart-rework
- **Агент**: —
- **Ветка**: —
