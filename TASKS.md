...
### TASK-108: Рефакторинг хранения состояния
- **Статус**: open
- **Приоритет**: P2
- **Этап**: N
- **Зависимости**: TASK-103
- **Файлы (новые)**: —
- **Файлы (modify)**: forgerace/tasks.py, forgerace/pipeline.py
- **Интеграция**: изменить хранение `last_attempts` на append-only лог в `TASKS.md`
- **Описание**: Перевести хранение `last_attempts` на append-only лог в `TASKS.md`. Убрать `rework_count` как отдельное поле, вычислять динамически.
- **Запрещено**: использовать глобальное состояние для `rework_count`
- **Проверка**: `make check`
- **Критерий готовности**: успешная запись истории попыток в `TASKS.md`
- **Дискуссия**: championship-smart-rework
- **Агент**: devstral
- **Ветка**: —
