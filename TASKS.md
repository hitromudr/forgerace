...
### TASK-127: Интеграция Dashboard в CLI и фоновый цикл обновления
- **Статус**: open
- **Приоритет**: P1
- **Этап**: 1
- **Зависимости**: TASK-126
- **Файлы (новые)**: —
- **Файлы (modify)**: forgerace/cli.py, forgerace/pipeline.py
- **Интеграция**: `from forgerace.dashboard import Dashboard`
- **Описание**: Добавить в CLI флаг `--tui`. При его активации инициализировать `Dashboard` и запустить фоновый механизм обновления (каждые 5 секунд), который считывает состояние `TaskQueue` и `Pipeline` (время, затраты, статусы). Обеспечить подавление обычных логов в режиме TUI или их перенаправление, чтобы они не разрушали структуру таблицы.
- **Запрещено**: Блокировать основной поток выполнения (pipeline) операциями отрисовки. Допускать перемешивание обычного текстового вывода с TUI-интерфейсом.
- **Проверка**: `python3 forgerace.py run --tui examples/example.toml`
- **Критерий готовности**: При запуске с флагом `--tui` пользователь видит живую таблицу прогресса, которая обновляется автоматически до завершения всех задач.
