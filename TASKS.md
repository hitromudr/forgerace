...
### TASK-134: Расширение моделей данных Config и Task
- **Статус**: open
- **Приоритет**: P1
- **Этап**: 1
- **Зависимости**: —
...
- **Файлы (modify)**: forgerace/config.py, forgerace/tasks.py
- **Интеграция**: —
- **Описание**: Добавить параметр `max_reworks: int = 3` в класс `Config` (секция `limits`) и поле `rework_count: int = 0` в dataclass `Task`. Это необходимо, так как, несмотря на отметки "done" в TASKS.md, эти поля отсутствуют в текущем коде.
- **Критерий готовности**: Объекты `Config` и `Task` успешно инициализируются с новыми полями.

### TASK-135: Обновление механизмов парсинга и сохранения в tasks.py
- **Статус**: open
- **Приоритет**: P1
- **Этап**: 1
- **Зависимости**: TASK-134
- **Файлы (новые)**: —
- **Файлы (modify)**: forgerace/tasks.py
- **Интеграция**: —
- **Описание**: Обновить функцию `parse_tasks` для извлечения `rework_count` из Markdown (регулярное выражение для `**Переделки**: N`) и функцию `update_task_status` для записи этого поля обратно в `TASKS.md`.
- **Критерий готовности**: Значение счетчика `rework_count` корректно считывается из файла и сохраняется в нем после обновлений.

### TASK-136: Реализация инкрементирования и логики STUCK в pipeline.py
- **Статус**: open
- **Приоритет**: P1
- **Этап**: 2
- **Зависимости**: TASK-135
- **Файлы (новые)**: —
- **Файлы (modify)**: forgerace/pipeline.py
- **Интеграция**: —
- **Описание**: В функциях `execute_task_competitive` и `execute_task_single` добавить инкрементирование `task.rework_count` перед каждым вызовом `send_to_rework`. Реализовать проверку: если `task.rework_count >= cfg.max_reworks`, задача переводится в статус `stuck` и выполнение прерывается.
- **Критерий готовности**: Счетчик увеличивается при каждой доработке; при достижении лимита задача помечается как `stuck`.
...
- **Приоритет**: P1
- **Этап**: 2
- **Зависимости**: TASK-136, TASK-095, TASK-096
- **Файлы (новые)**: —
- **Файлы (modify)**: forgerace/pipeline.py
...
### TASK-099: Чекпоинт — починить check
- **Статус**: open
- **Приоритет**: P1
- **Этап**: 4
- **Зависимости**: —
...
- **Файлы (modify)**: —
- **Интеграция**: —
- **Описание**: `make check` не проходит. Ошибка: make: *** Нет правила для сборки цели «check».  Останов.. Создать Makefile с таргетом check если его нет, исправить все проблемы.
- **Критерий готовности**: `make check` проходит с exit code 0
- **Дискуссия**: —
- **Агент**: —
- **Ветка**: —

### TASK-101: Integration & Config
- **Статус**: open
- **Приоритет**: P1
- **Этап**: 1
- **Зависимости**: —
- **Файлы (новые)**: —
- **Файлы (modify)**: forgerace/config.py, forgerace/pipeline.py
- **Интеграция**: расширение схемы Config и обновление логов в Pipeline
- **Описание**: Добавить в конфигурацию поддержку списка моделей-ревьюеров (`reviewers_pool`). Обновить пайплайн для вывода в CLI детальных логов по каждому вердикту (название модели + её оценка + краткий статус).
- **Запрещено**: хардкодить список моделей, игнорировать существующие параметры `review_consensus` и `min_reviewers`.
- **Проверка**: python3 forgerace/config.py && ruff check forgerace/pipeline.py
- **Критерий готовности**: Наличие возможности задать список моделей в конфиге и видеть вердикт каждой модели в выводе CLI.
- **Дискуссия**: championship-v2-ensemble
- **Агент**: aider-qwen
- **Ветка**: —

### TASK-102: Consensus Logic & Veto
- **Статус**: open
- **Приоритет**: P1
- **Этап**: 2
- **Зависимости**: TASK-100
- **Файлы (новые)**: —
- **Файлы (modify)**: forgerace/review.py
- **Интеграция**: вызов в `validate_review` для агрегации результатов
- **Описание**: Реализовать логику консенсуса. Если `review_consensus=True`, внедрить механизм "вето": если хотя бы один агент возвращает `NEEDS_WORK`, итоговый статус всей проверки — `NEEDS_WORK`. Объединить текстовые комментарии от всех моделей в единый отчет.
- **Запрещено**: пропускать мерж при наличии хотя бы одного вето, терять комментарии моделей, проголосовавших против.
- **Проверка**: pytest tests/test_review.py -k "consensus_veto"
- **Критерий готовности**: Корректное применение правила вето: любое `NEEDS_WORK` блокирует задачу.
- **Дискуссия**: championship-v2-ensemble
- **Агент**: gemini
- **Ветка**: —

### TASK-103: Fallback & Resilience
- **Статус**: open
- **Приоритет**: P1
- **Этап**: 2
- **Зависимости**: TASK-100
- **Файлы (новые)**: —
- **Файлы (modify)**: forgerace/review.py
- **Интеграция**: обработка исключений при недостаточном количестве ответов
- **Описание**: Реализовать механизм Fallback: если количество успешно ответивших моделей меньше `min_reviewers`, система должна автоматически инициировать одиночное ревью основной моделью (gemini) для обеспечения непрерывности процесса.
- **Запрещено**: бесконечные циклы ретраев, падение пайплайна при недоступности внешних API (LiteLLM/бесплатных моделей).
- **Проверка**: pytest tests/test_review.py -k "fallback_mechanism"
- **Критерий готовности**: При сбое всех моделей ансамбля система автоматически переходит в режим Single Review и завершает проверку.
- **Дискуссия**: championship-v2-ensemble
- **Агент**: aider-devstral
- **Ветка**: —

### TASK-105: Реализация API метода для Post-mortem анализа
- **Статус**: open
- **Приоритет**: P1
- **Этап**: 1
- **Зависимости**: —
...
- **Файлы (modify)**: forgerace/agents.py
- **Интеграция**: —
- **Описание**: Реализовать в `agents.py` функцию для вызова LLM-модели, которая анализирует список накопленных замечаний (`last_attempts`) и генерирует аналитический отчет (Post-mortem) о причинах неудачи.
- **Запрещено**: —
- **Проверка**: python3 -m pytest tests/test_agents.py
- **Критерий готовности**: В системе доступен метод для получения интеллектуального анализа причин зацикливания или провала задачи.
- **Дискуссия**: championship-v2-rework
- **Агент**: aider-devstral
- **Ветка**: —

### TASK-134: Рефакторинг структуры метаданных агента
- **Статус**: open
- **Приоритет**: P1
- **Этап**: 1
- **Зависимости**: —
- **Файлы (modify)**: forgerace/agents.py
- **Интеграция**: Добавить поле для хранения системных фреймов в структуре метаданных агента
- **Описание**: Модифицировать dataclass агента, добавив поле `system_frames` типа dict для хранения контекстов фреймов (`+architect`, `+theory`)
- **Критерий готовности**: Структура данных готова для хранения системных инструкций
- **Дискуссия**: championship-v2-preflight

### TASK-135: Реализация механизма передачи контекста
- **Статус**: open
- **Приоритет**: P1
- **Этап**: 1
- **Зависимости**: TASK-134
- **Файлы (modify)**: forgerace/agents.py
- **Интеграция**: Добавить метод инициализации системных фреймов в классе агента
- **Описание**: Реализовать функцию `initialize_system_frames()`, которая будет загружать инструкции для фреймов из конфигурации или внешних файлов
- **Критерий готовности**: Агент может инициализироваться с заданными системными фреймами
- **Дискуссия**: championship-v2-preflight

### TASK-136: Интеграция фреймов в процесс инициализации
- **Статус**: open
- **Приоритет**: P2
- **Этап**: 2
- **Зависимости**: TASK-135
- **Файлы (modify)**: forgerace/agents.py
- **Интеграция**: Модифицировать конструктор агента для поддержки системных фреймов
- **Описание**: Обновить метод `__init__` агента, чтобы он принимал параметр `system_frame` и инициализировал соответствующий контекст
- **Критерий готовности**: При создании агента можно указать системный фрейм (`+architect` или `+theory`)
- **Дискуссия**: championship-v2-preflight

### TASK-137: Тестирование и валидация
- **Статус**: open
- **Приоритет**: P3
- **Этап**: 3
- **Зависимости**: TASK-136
- **Файлы (modify)**: —
- **Интеграция**: Добавить тесты для проверки работы системных фреймов
- **Описание**: Написать unit-тесты, проверяющие корректную инициализацию агентов с разными фреймами и передачу контекста
- **Критерий готовности**: Все тесты проходят, агенты корректно работают с системными фреймами
- **Дискуссия**: championship-v2-preflight

### TASK-129: Интеграция с циклом реворка
- **Статус**: open
- **Приоритет**: P1
- **Этап**: 2
- **Зависимости**: TASK-137
- **Файлы (новые)**: — 
- **Файлы (modify)**: forgerace/pipeline.py
- **Интеграция**: Интегрировать обновление состояния задачи в цикл реворка
- **Описание**: Обновить функцию `send_to_rework` для вызова обновленной функции `update_task_status` и сохранения текста каждого нового замечания в список `task.last_attempts`
- **Критерий готовности**: При каждом повторном прогоне (rework) данные в объекте задачи корректно обновляются и накапливаются
- **Дискуссия**: championship-v2-rework
- **Агент**: — 
- **Ветка**: —

### TASK-130: Тестирование обновления состояния задачи
- **Статус**: open
- **Приоритет**: P1
- **Этап**: 2
- **Зависимости**: TASK-137, TASK-129
- **Файлы (новые)**: tests/test_pipeline.py
- **Файлы (modify)**: — 
- **Интеграция**: Добавить тесты для обновления состояния задачи
- **Описание**: Написать тесты для функции `update_task_status` и интеграции с циклом реворка
- **Критерий готовности**: Тесты проходят успешно
- **Дискуссия**: championship-v2-rework
- **Агент**: — 
- **Ветка**: —

### TASK-107: Логирование прогресса попыток (Attempt X/Y)
- **Статус**: open
- **Приоритет**: P2
- **Этап**: 3
- **Зависимости**: TASK-130
- **Файлы (новые)**: —
- **Файлы (modify)**: forgerace/pipeline.py
- **Интеграция**: —
- **Описание**: Добавить информационное сообщение в лог перед началом каждой итерации реворка в формате "Attempt X/Y", где X — текущая попытка, а Y — максимальное количество попыток.
- **Запрещено**: —
- **Проверка**: Запустить выполнение задачи с заведомой ошибкой и проверить наличие лога "Attempt X/Y" в выводе.
- **Критерий готовности**: Пользователь видит в консоли текущий прогресс попыток исправления кода.
- **Дискуссия**: championship-v2-rework
- **Агент**: —
- **Ветка**: —

### TASK-108: Реализация Stuck Detection (детектор зацикливания)
- **Статус**: open
- **Приоритет**: P1
- **Этап**: 3
- **Зависимости**: TASK-130
...
- **Файлы (modify)**: forgerace/pipeline.py
- **Интеграция**: —
- **Описание**: Реализовать проверку последних 3-х записей в `task.last_attempts`. Если замечания повторяются или идентичны (stuck state), прерывать цикл реворка досрочно.
- **Запрещено**: Использовать тяжелые библиотеки для эмбеддингов или косинусного сходства; достаточно простого строкового сравнения или нормализации текста.
- **Проверка**: python3 -m pytest tests/test_pipeline.py (тест на обнаружение stuck)
- **Критерий готовности**: Пайплайн автоматически останавливается, если агент получает три одинаковых замечания подряд.
- **Дискуссия**: championship-v2-rework
- **Агент**: gemini
- **Ветка**: —

### TASK-109: Интеграция вызова Post-mortem отчета
- **Статус**: open
- **Приоритет**: P2
- **Этап**: 4
- **Зависимости**: TASK-105, TASK-108
- **Файлы (новые)**: —
- **Файлы (modify)**: forgerace/pipeline.py
- **Интеграция**: Импорт метода из forgerace.agents
- **Описание**: Добавить вызов генерации Post-mortem отчета при срабатывании Stuck Detection или при окончательном исчерпании лимита попыток реворка. Выводить полученный отчет в консоль.
- **Запрещено**: —
- **Проверка**: Спровоцировать ситуацию stuck и убедиться, что в конце выводится аналитический отчет.
- **Критерий готовности**: При провале задачи пайплайн предоставляет пользователю анализ причин неудачи от API-модели.
- **Дискуссия**: championship-v2-rework
- **Агент**: —
- **Ветка**: —

### TASK-111: Расширение конфигурации для Pre-flight Analysis
- **Статус**: open
- **Приоритет**: P1
- **Этап**: 1
- **Зависимости**: —
...
- **Файлы (modify)**: forgerace/config.py
- **Интеграция**: —
- **Описание**: Добавить в класс `Config` новые поля: `preflight` (boolean, включение функции), `preflight_agent` (строка, имя модели для анализа, по умолчанию gemini или aider-qwen), и `preflight_mode` (выбор между strict/auto).
- **Запрещено**: Хардкодить список разрешенных агентов внутри логики — использовать гибкую проверку в конфиге.
- **Проверка**: `ruff check forgerace/config.py && python3 -c "from forgerace.config import Config; c=Config(); print(c.preflight)"`
- **Критерий готовности**: Новые параметры конфигурации доступны во всем приложении и корректно считываются из .toml файлов.
- **Дискуссия**: championship-v2-preflight
- **Агент**: gemini
- **Ветка**: task/task-111-rasshirenie-konfiguratsii-dlya-pre-fligh-gemini

### TASK-128: Реализация поддержки системных фреймов
- **Статус**: open
- **Приоритет**: P2
- **Этап**: 1
- **Зависимости**: —
- **Файлы (новые)**: —
- **Файлы (modify)**: forgerace/agents.py
- **Интеграция**: Добавить поддержку системных фреймов `+architect` и `+theory` в существующий код.
- **Описание**: Реализовать механизм передачи контекста "планировщика" в метаданные агента для поддержки системных фреймов.
- **Критерий готовности**: Агенты инициализируются с правильными системными инструкциями в зависимости от выбранного фрейма.
- **Дискуссия**: championship-v2-preflight
- **Агент**: —
- **Ветка**: —

### TASK-129: Обработка метаданных агента
- **Статус**: open
- **Приоритет**: P2
- **Этап**: 1
- **Зависимости**: TASK-137
- **Файлы (новые)**: —
- **Файлы (modify)**: forgerace/agents.py
- **Интеграция**: Обработка метаданных агента для поддержки системных фреймов.
- **Описание**: Реализовать логику обработки метаданных агента для поддержки системных фреймов.
- **Критерий готовности**: Метаданные агента правильно обрабатываются и используются для инициализации агентов.
- **Дискуссия**: championship-v2-preflight
- **Агент**: —
- **Ветка**: —

### TASK-130: Тестирование поддержки системных фреймов
- **Статус**: open
- **Приоритет**: P2
- **Этап**: 1
- **Зависимости**: TASK-137, TASK-129
- **Файлы (новые)**: —
- **Файлы (modify)**: forgerace/agents.py
- **Интеграция**: Тестирование поддержки системных фреймов.
- **Описание**: Реализовать тесты для проверки корректности поддержки системных фреймов.
- **Критерий готовности**: Тесты проходят успешно, подтверждая корректность поддержки системных фреймов.
- **Дискуссия**: championship-v2-preflight
- **Агент**: —
- **Ветка**: —

### TASK-113: Интеграция Pre-flight Analysis в основной Pipeline
- **Статус**: open
- **Приоритет**: P1
- **Этап**: 2
- **Зависимости**: TASK-110, TASK-111
- **Файлы (новые)**: —
- **Файлы (modify)**: forgerace/pipeline.py
- **Интеграция**: Вызов `TaskPlanner.analyze` перед запуском цикла кодирования.
- **Описание**: Внедрить точку прерывания в `pipeline.py`. Если `config.preflight` активен, запускать этап анализа. Результат анализа (JSON) должен логироваться в отдельный файл и инжектироваться в промпт кодинг-агента. В режиме `strict` блокировать выполнение кодинга, если валидация плана провалилась.
- **Запрещено**: Нарушать текущую логику работы `pipeline.py` без активного флага preflight, игнорировать ошибки валидации плана.
- **Проверка**: `ruff check forgerace/pipeline.py`
- **Критерий готовности**: При запуске `forgerace` с включенным preflight агент получает детальный план работы, а в логах проекта появляется файл с анализом задачи.
- **Дискуссия**: championship-v2-preflight
- **Агент**: —
- **Ветка**: —

### TASK-115: Диспетчер выбора режима выполнения в Pipeline
- **Статус**: open
- **Приоритет**: P1
- **Этап**: 2
- **Зависимости**: TASK-114
- **Файлы (новые)**: —
- **Файлы (modify)**: forgerace/pipeline.py
- **Интеграция**: импорт настроек из config.py
- **Описание**: Модернизировать `execute_task_competitive` или создать обертку-диспетчер. Реализовать логику: если `complexity > 3`, запускать всех (competitive). Если `complexity <= 3` и в задаче явно указан агент — запускать только его.
- **Запрещено**: отключать competitive mode для задач со сложностью выше 3; использовать `networkidle` в тестах (если будут создаваться).
- **Проверка**: запустить pipeline на тестовой задаче с complexity=4 и убедиться, что стартуют все агенты.
- **Критерий готовности**: Система корректно переключается между competitive и assigned режимами в зависимости от сложности и метаданных задачи.
- **Дискуссия**: championship-v2-distributed
- **Агент**: —
- **Ветка**: —

### TASK-116: Реализация Round-robin распределения агентов
- **Статус**: open
- **Приоритет**: P1
- **Этап**: 3
- **Зависимости**: TASK-115
- **Файлы (новые)**: —
- **Файлы (modify)**: forgerace/pipeline.py
- **Интеграция**: —
- **Описание**: Реализовать алгоритм циклического распределения (round-robin) для задач, где `complexity <= 3` и агент не указан. Использовать список надежных агентов из конфига. Состояние очереди (кто следующий) должно сохраняться в рамках одного запуска пайплайна.
- **Запрещено**: использовать goose-llama и goose-devstral для автоматического распределения из-за их ненадежности.
- **Проверка**: запустить пачку из 3-5 простых задач без указания агентов и убедиться, что они распределяются между gemini, aider-devstral и aider-qwen по очереди.
- **Критерий готовности**: Задачи без назначенных агентов равномерно распределяются между доступными исполнителями.
- **Дискуссия**: championship-v2-distributed
- **Агент**: —
- **Ветка**: —

### TASK-117: Инструментация и детальное логирование назначений
- **Статус**: open
- **Приоритет**: P2
- **Этап**: 3
- **Зависимости**: TASK-115
...
- **Интеграция**: —
- **Описание**: Добавить подробное логирование в процесс выбора агента. Лог должен содержать: ID задачи, выбранного агента и причину выбора (явное указание, порог сложности или round-robin).
- **Запрещено**: выводить чувствительные данные или полные конфиги в логи.
- **Проверка**: проверить вывод в консоль/лог-файл при работе пайплайна в разных режимах.
- **Критерий готовности**: Пользователь видит в логах четкую причину, почему конкретный агент был назначен на задачу.
- **Дискуссия**: championship-v2-distributed
- **Агент**: —
- **Ветка**: —

### TASK-131: Реализация модуля BenchmarkStore
- **Статус**: open
- **Приоритет**: P1
- **Этап**: 2
- **Зависимости**: —
- **Файлы (новые)**: forgerace/benchmark.py
- **Файлы (modify)**: —
- **Интеграция**: `from forgerace.benchmark import BenchmarkStore, BenchmarkRecord`
- **Описание**: Реализовать инфраструктуру для хранения метрик. 1. `BenchmarkRecord` (dataclass): `task_id`, `agent`, `duration_sec`, `total_cost_usd`, `review_rounds`, `lines_changed`. 2. `BenchmarkStore`: класс с методом `save(record)`, обеспечивающий потокобезопасную запись (через `threading.Lock`) в JSON-массив в файл `.agents/benchmark.json`. Если файл не существует, создавать его.
- **Критерий готовности**: Модуль позволяет сохранять записи, данные в `.agents/benchmark.json` корректно дописываются и представляют собой валидный JSON-список.
- **Дискуссия**: championship-v2-benchmark

### TASK-132: Инструментация Pipeline (Single mode)
- **Статус**: open
- **Приоритет**: P1
- **Этап**: 2
- **Зависимости**: TASK-131
- **Файлы (новые)**: —
- **Файлы (modify)**: forgerace/pipeline.py
- **Интеграция**: импорт BenchmarkStore
- **Описание**: Интегрировать сбор метрик в функцию `execute_task_single`. 1. Замерять время выполнения всей задачи (через `time.perf_counter()`). 2. Добавить счетчик `review_rounds` в цикл ревью. 3. После успешного мерджа вычислить `lines_changed` через `git diff --shortstat HEAD~1` (или сравнение с `cfg.dev_branch`) в рабочей директории агента. 4. Сформировать `BenchmarkRecord` (используя `result.usage` для стоимости) и вызвать `BenchmarkStore.save()`.
- **Критерий готовности**: После прогона любой задачи в одиночном режиме в `benchmark.json` появляется запись с реальными данными о времени, стоимости и правках.
- **Дискуссия**: championship-v2-benchmark

### TASK-133: Инструментация Pipeline (Competitive mode)
- **Статус**: open
- **Приоритет**: P1
- **Этап**: 2
- **Зависимости**: TASK-131
...
- **Файлы (modify)**: forgerace/pipeline.py
- **Интеграция**: импорт BenchmarkStore
- **Описание**: Интегрировать сбор метрик в функцию `execute_task_competitive`. 1. Замерять общее время прогона. 2. Считать раунды ревью для победившего агента (включая фазу параллельного ревью). 3. Суммировать `total_cost_usd` всех участвовавших агентов для этой задачи. 4. После определения победителя вычислять `lines_changed` и сохранять итоговую запись через `BenchmarkStore`.
- **Критерий готовности**: При конкурентном режиме в бенчмарк записываются метрики победителя с учетом суммарной стоимости попыток всех агентов.
- **Дискуссия**: championship-v2-benchmark
...
- **Файлы (новые)**: —
- **Файлы (modify)**: forgerace/cli.py
- **Интеграция**: `from forgerace.benchmark import BenchmarkStore`
- **Описание**: Добавить в CLI подкоманду `benchmark`. Без аргументов — вывод таблицы с агрегированными метриками по каждому агенту (успех %, ср. время, ср. стоимость, ср. объем правок). С флагом `--agent <name>` — вывод детальной истории задач для конкретного агента. Использовать `tabulate` или `rich` для форматирования.
- **Запрещено**: не хардкодить список агентов (брать из логов), не падать при отсутствии файла `benchmark.json` (выводить понятное сообщение).
- **Проверка**: `./forgerace.py benchmark --help`
- **Критерий готовности**: Пользователь может просмотреть сравнительную таблицу производительности агентов в терминале.
- **Дискуссия**: championship-v2-benchmark
- **Агент**: aider-qwen
- **Ветка**: —
...
- **Этап**: 2
- **Зависимости**: TASK-118
- **Файлы (новые)**: tests/test_benchmark.py
- **Файлы (modify)**: —
- **Интеграция**: —
- **Описание**: Реализовать тесты для модуля `BenchmarkStore`. Покрыть: создание записи, чтение данных, корректность агрегации (среднее, медиана), обработку конкурентных записей (mock filelock).
- **Запрещено**: не использовать реальный `.agents/benchmark.json` (использовать временные файлы pytest), не зависеть от внешних API.
- **Проверка**: `pytest tests/test_benchmark.py`
- **Критерий готовности**: 100% прохождение тестов логики хранения и обработки данных.
- **Дискуссия**: championship-v2-benchmark
- **Агент**: —
- **Ветка**: —

### TASK-123: Реализация запуска тестов после мерджа
- **Статус**: open
- **Приоритет**: P1
- **Этап**: 2
- **Зависимости**: TASK-122
- **Файлы (новые)**: —
- **Файлы (modify)**: forgerace/merge.py
- **Интеграция**: Вызов новой логики запуска внутри функции мерджа или сразу после неё.
- **Описание**: Внедрить запуск `test_command` после успешного завершения `git merge`. Реализовать захват stdout и stderr выполнения тестов и передачу кода возврата выше по стеку в пайплайн.
- **Запрещено**: Не подавлять код возврата (exit code) тестов; не игнорировать ошибки, если команда теста пустая.
- **Проверка**: `pytest tests/test_merge_runner.py` (создать мок-тест для проверки вызова команды)
- **Критерий готовности**: После мерджа автоматически выполняется команда тестирования, лог выполнения доступен для анализа.
- **Дискуссия**: championship-v2-testrunner
- **Агент**: —
- **Ветка**: —

### TASK-124: Автоматическое создание fix-задач в пайплайне
- **Статус**: open
- **Приоритет**: P1
- **Этап**: 2
- **Зависимости**: TASK-122
- **Файлы (новые)**: —
- **Файлы (modify)**: forgerace/pipeline.py
- **Интеграция**: Импорт функционала создания задач из `forgerace/tasks.py`.
- **Описание**: Добавить в логику пайплайна проверку результата выполнения тестов. Если тесты упали и `auto_fix_on_fail` равен true, создать новую задачу в очереди (TASKS.md) с описанием, содержащим логи упавших тестов.
- **Запрещено**: Не создавать задачу, если тесты прошли (exit code 0); не создавать задачу, если `auto_fix_on_fail` отключен в конфиге.
- **Проверка**: `pytest tests/test_pipeline_autofix.py`
- **Критерий готовности**: При провале тестов в списке задач автоматически появляется новая задача на исправление с контекстом ошибки.
- **Дискуссия**: championship-v2-testrunner
- **Агент**: —
- **Ветка**: —

### TASK-125: Логирование результатов тестирования и создания задач
- **Статус**: open
- **Приоритет**: P1
- **Этап**: 2
- **Зависимости**: TASK-123, TASK-124
- **Файлы (новые)**: —
- **Файлы (modify)**: forgerace/utils.py, forgerace/pipeline.py
- **Интеграция**: —
- **Описание**: Реализовать подробное логирование: какие именно тесты упали (парсинг вывода pytest) и ID/название созданной fix-задачи.
- **Запрещено**: —
- **Проверка**: `grep "Test failure" forgerace.log` (после намеренного завала тестов)
- **Критерий готовности**: В консоли и лог-файле отображается статус тестов и подтверждение создания задачи на исправление.
- **Дискуссия**: championship-v2-testrunner
- **Агент**: —
- **Ветка**: —

### TASK-131: Реализация модуля forgerace/dashboard.py
- **Статус**: open
- **Приоритет**: P1
- **Этап**: 1
- **Зависимости**: —
- **Файлы (новые)**: forgerace/dashboard.py
- **Файлы (modify)**: —
- **Интеграция**: Экспорт класса `Dashboard`
- **Описание**: Создать класс `Dashboard` для отрисовки TUI. Он должен принимать состояние `TaskQueue` и данные о запущенных агентах (время, файлы, затраты) и выводить их в виде живой таблицы. Рекомендуется использовать `rich.live` для эффективного обновления консоли.
- **Критерий готовности**: Код `from forgerace.dashboard import Dashboard` работает; класс имеет метод `update()`, который корректно обновляет таблицу в терминале.
- **Дискуссия**: championship-v2-dashboard
- **Агент**: —
- **Ветка**: —

### TASK-132: Интеграция флага --tui и подавление логов
- **Статус**: open
- **Приоритет**: P1
- **Этап**: 2
- **Зависимости**: TASK-131
- **Файлы (новые)**: —
- **Файлы (modify)**: forgerace/cli.py, forgerace/utils.py
- **Интеграция**: Передача параметра `use_tui` в пайплайн
- **Описание**: Добавить `--tui` в `argparse`. В `forgerace/utils.py` (или `cli.py`) изменить `setup_logging` так, чтобы при включенном TUI логи не писались в `stdout/stderr` (предотвращая "разрыв" таблицы), а направлялись только в файл или специальный буфер.
- **Критерий готовности**: При запуске с `--tui` стандартный вывод логов отсутствует, инициализируется пустой интерфейс Dashboard.
- **Дискуссия**: championship-v2-dashboard
- **Агент**: —
- **Ветка**: —

### TASK-133: Фоновый цикл обновления Dashboard в пайплайне
- **Статус**: open
- **Приоритет**: P1
- **Этап**: 2
- **Зависимости**: TASK-131
- **Файлы (новые)**: —
- **Файлы (modify)**: forgerace/pipeline.py
- **Интеграция**: Запуск потока в `run_pipeline`
- **Описание**: В `forgerace/pipeline.py` реализовать функцию `_dashboard_loop`, которая запускается в отдельном daemon-потоке при `cfg.tui = True`. Цикл должен каждые 5 секунд опрашивать `TaskQueue` и `_active_agents`, агрегировать данные (затраты из `TokenUsage`, статусы задач) и вызывать `dashboard.update()`.
- **Критерий готовности**: В режиме TUI пользователь видит автоматическое обновление статусов и времени работы агентов без ручного ввода.
- **Дискуссия**: championship-v2-dashboard
- **Агент**: —
- **Ветка**: —

### TASK-134: Реализация модуля BenchmarkStore
- **Статус**: open
- **Приоритет**: P1
- **Этап**: 2
- **Зависимости**: —
...
### TASK-135: Инструментация Pipeline (Single mode)
- **Статус**: open
- **Приоритет**: P1
- **Этап**: 2
- **Зависимости**: TASK-134
...
- **Файлы (modify)**: forgerace/pipeline.py
- **Интеграция**: импорт BenchmarkStore
- **Описание**: Интегрировать сбор метрик в функцию `execute_task_single`. 1. Замерять время выполнения всей задачи (через `time.perf_counter()`). 2. Добавить счетчик `review_rounds` в цикл ревью. 3. После успешного мерджа вычислить `lines_changed` через `git diff --shortstat` в рабочей директории агента (сравнение с `cfg.dev_branch`). 4. Сформировать `BenchmarkRecord` (используя `result.usage` для стоимости) и вызвать `BenchmarkStore.save()`.
- **Критерий готовности**: После прогона любой задачи в одиночном режиме в `benchmark.json` появляется запись с реальными данными о времени, стоимости и правках.
- **Дискуссия**: championship-v2-benchmark
- **Агент**: gemini
- **Ветка**: —

### TASK-136: Инструментация Pipeline (Competitive mode)
- **Статус**: open
- **Приоритет**: P1
- **Этап**: 2
- **Зависимости**: TASK-134
