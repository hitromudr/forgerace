...
### TASK-090: Интеграция функции build_rework_prompt в build_prompt
- **Статус**: open
- **Приоритет**: P1
- **Этап**: 2
- **Зависимости**: TASK-089
- **Файлы (новые)**: —
- **Файлы (modify)**: forgerace/agents.py
- **Интеграция**: Добавить вызов функции `build_rework_prompt` в `build_prompt`
- **Описание**: Интегрировать функцию `build_rework_prompt` в `build_prompt`, чтобы агент видел контекст прошлых неудачных попыток и замечания ревьюера
- **Критерий готовности**: В системном промпте агента должен появляться блок с описанием предыдущих итераций ревью
- **Дискуссия**: —
