...
VERDICT_NEEDS_REWORK = "NEEDS_REWORK"
VERDICT_NEEDS_WORK = VERDICT_NEEDS_REWORK  # backward-compat alias
VERDICT_REJECTED = "REJECTED"

_VERDICT_ALIASES = {
    "NEEDS_WORK": VERDICT_NEEDS_REWORK,
    "NEEDS_REWORK": VERDICT_NEEDS_REWORK,
...
REVIEW_SCHEMA = {
    "verdict": {
        "type": "str",
        "allowed": [VERDICT_APPROVED, VERDICT_NEEDS_REWORK, VERDICT_REJECTED],
        "aliases": {"NEEDS_WORK": VERDICT_NEEDS_REWORK},
        "is_terminal": {
            VERDICT_APPROVED: True,
            VERDICT_NEEDS_REWORK: False,
    ...
    "issues": {
        "type": "list[dict]",
        "item_fields": ["severity", "text"],
    },
}


def _parse_issues(raw_issues: list | str | None) -> list[dict]:
    """Parse issues from list of '[severity] text' strings into normalized dicts."""
    if not raw_issues:
        return []
    if isinstance(raw_issues, str):
        raw_issues = [line.strip() for line in raw_issues.strip().splitlines() if line.strip()]
    result = []
    ...
    for item in raw_issues:
        ...
        if m:
            result.append({"severity": m.group(1).lower(), "text": m.group(2).strip()})
        else:
            result.append({"severity": "info", "text": str(item).strip()})
    return result


def validate_review(data: dict) -> tuple[bool, str]:
    """Validate and normalize review data in-place.

    Returns (ok, error_message). On success error_message is empty.
    Mutates *data*: coerces confidence to int, normalizes verdict alias,
    parses issues into list[dict].
    """
    # --- verdict ---
    verdict_raw = data.get("verdict")
    if not verdict_raw or not isinstance(verdict_raw, str):
        return False, "missing or invalid 'verdict'"
    verdict_upper = verdict_raw.strip().upper()
    verdict = _VERDICT_ALIASES.get(verdict_upper)
    if verdict is None:
        return False, f"unknown verdict '{verdict_raw}', expected one of {list(_VERDICT_ALIASES.keys())}"
    ...
    if conf_raw is not None:
        ...
        try:
            ...
            return False, f"confidence must be int or float, got {type(conf_raw).__name__}"
        lo, hi = REVIEW_SCHEMA["confidence"]["confidence_range"]
        if conf < lo or conf > hi:
            return False, f"confidence {conf} out of range [{lo}, {hi}]"
        data["confidence"] = conf

    # --- issues ---
    data["issues"] = _parse_issues(data.get("issues"))

    # --- cross-field validation ---
    has_critical = any(
        iss.get("severity") in ("critical", "blocker")
        for iss in data["issues"]
    )
    if verdict == VERDICT_APPROVED and has_critical:
        return False, "APPROVED verdict incompatible with critical/blocker issues"

    if verdict == VERDICT_REJECTED and not data["issues"]:
        return False, "REJECTED verdict requires at least one issue"
