...
def _post_resolve(filepath: Path):
    ...
    # Collect real project structure for the prompt
    _top_dirs = []
    _top_files = []
    for item in sorted(cfg.root_dir.iterdir()):
        if item.name.startswith("."):
            continue
        if item.is_dir():
            _top_dirs.append(f"  {item.name}/")
        elif item.is_file() and item.suffix in (".py", ".toml", ".md", ".yaml", ".yml"):
            _top_files.append(f"  {item.name}")
    _project_tree = "\n".join(_top_dirs[:15] + _top_files[:10])

    prompt = f"""Прочитай закрытую дискуссию и сгенерируй блок задач для TASKS.md.

## Структура проекта (корень: {cfg.root_dir})
{_project_tree}
...
### TASK-XXX: Название
- **Статус**: open
- **Описание**: ПОДРОБНОЕ описание. Включи: (1) какие функции/классы создать с сигнатурами, (2) какие данные на вход/выход, (3) какие существующие модули использовать. Агент-исполнитель НЕ видит дискуссию — всё что ему нужно должно быть в описании.
- **Файлы (новые)**: путь/к/файлу.py (относительно корня проекта, НЕ src/)
- **Файлы (modify)**: путь (только если меняет существующий файл)
- **Зависимости**: TASK-YYY (только если реально зависит)
- **Запрещено**: конкретные антипаттерны (из дискуссии). Пропусти если нет.
- **Проверка**: команда верификации. Должна быть конкретной и запускаемой.
- **Дискуссия**: {topic}

НЕ ПИШИ поля со значением "—". Пропускай пустые поля. Пиши только заполненные.
...
- Одна задача = одна логическая единица работы (новый модуль, интеграция в CLI, тест). НЕ дроби один файл на отдельные функции — если весь результат это один файл, это ОДНА задача.
- Максимум 3 задачи. Лучше 1-2 крупные чем 8 мелких.
- Зависимости: ТОЛЬКО реальные (задача B использует код из задачи A). НЕ ставь линейную цепочку.
- Пути файлов: используй РЕАЛЬНЫЕ пути проекта (см. структуру выше). НЕ выдумывай каталог src/ если его нет.
- Пиши на русском
