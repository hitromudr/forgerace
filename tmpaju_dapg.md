...
## Твоя задача: TASK-035 — Реализовать команду ./fr task list

Создать функцию `list_tasks()` в `forgerace/task_cmd.py`, которая выводит компактную таблицу задач: ID, статус, агент, название. Использовать `parse_tasks()` из `tasks.py` для парсинга `TASKS.md`.
...
## Интеграция
Добавить вызов `list_tasks()` в `cli.py`

## Критерий готовности
Команда `./fr task list` выводит компактную таблицу задач
